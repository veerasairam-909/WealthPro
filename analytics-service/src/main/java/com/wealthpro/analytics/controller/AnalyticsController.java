...
public interface AnalyticsController {
    ...
    ResponseEntity<List<PerformanceRecordResponseDTO>> getPerformance(@PathVariable Long accountId);
    ...
    ResponseEntity<List<RiskMeasureResponseDTO>> getRiskMeasures(@PathVariable Long accountId);
    ...
    ResponseEntity<List<RiskMeasureResponseDTO>> runRiskAssessment(@PathVariable Long accountId);

    // =================== Compliance ===================

    @Operation(summary = "Get compliance breaches by account ID")
    ResponseEntity<List<ComplianceBreachResponseDTO>> getBreaches(@PathVariable Long accountId);

    @Operation(summary = "Run compliance scan",
            description = "Detects concentration, exposure, and restricted security violations")
    ResponseEntity<List<ComplianceBreachResponseDTO>> runComplianceScan(@PathVariable Long accountId);

    @Operation(summary = "Acknowledge a breach", description = "Moves OPEN → ACKNOWLEDGED")
    ResponseEntity<ComplianceBreachResponseDTO> acknowledgeBreach(@PathVariable Long breachId);

    @Operation(summary = "Close a breach", description = "Moves ACKNOWLEDGED → CLOSED")
    ResponseEntity<ComplianceBreachResponseDTO> closeBreach(@PathVariable Long breachId);

    // =================== Dashboard ===================

    @Operation(summary = "Get account dashboard",
            description = "Combined view of performance, risk, and open compliance breaches")
    ResponseEntity<AccountDashboardDTO> getAccountDashboard(@PathVariable Long accountId);
}
