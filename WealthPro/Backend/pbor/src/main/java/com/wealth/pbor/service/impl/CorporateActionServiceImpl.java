...
public class CorporateActionServiceImpl implements CorporateActionService {
    ...
    public CorporateActionResponse createCorporateAction(CorporateActionRequest request) {
        ...
        // ── Feign: Send corporate action notification ────────────────────────
        try {
            NotificationRequestDTO notification = new NotificationRequestDTO(
                    account.getClientId(),
                    "Corporate Action Alert: " + request.getCaType() + " processed for security ID "
                            + request.getSecurityId() + ". Pay date: " + request.getPayDate(),
                    "CorporateAction"
            );
            notificationFeignClient.sendNotification(notification);
            log.info("[FEIGN] Corporate action notification sent to NOTIFICATIONS-SERVICE for clientId={}",
                    account.getClientId());
        } catch (FeignException e) {
            log.warn("[FEIGN] Could not send corporate action notification: {}", e.getMessage());
        }
        // ────────────────────────────────────────────────────────────────────
