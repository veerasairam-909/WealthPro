...
public class RecommendationServiceImpl implements RecommendationService {
    ...
    public RecommendationResponse createRecommendation(RecommendationRequest request) {
        ...
        try {
            ...
            log.warn("[FEIGN] Could not validate client from WEALTHPRO-SERVICE: {}", e.getMessage());
        }
        // ────────────────────────────────────────────────────────────────────

        // ── Feign: Fetch risk profile to cross-check riskClass alignment ─────
        try {
            var riskProfile = wealthproFeignClient.getRiskProfileByClientId(request.getClientId());
            log.info("[FEIGN] Risk profile fetched from WEALTHPRO-SERVICE → clientId={}, riskScore={}",
                    request.getClientId(), riskProfile.getRiskScore());
        } catch (FeignException e) {
            log.warn("[FEIGN] Could not fetch risk profile from WEALTHPRO-SERVICE: {}", e.getMessage());
        }
        // ────────────────────────────────────────────────────────────────────
        ...
        if (matchingPortfolios.isEmpty()) {
            throw new ResourceNotFoundException("No active model portfolio found for risk class: "
                    + request.getRiskClass() + ". Please ask Admin to create one.");
        }
    ...
    public RecommendationResponse updateRecommendation(Long id, RecommendationRequest request) {
        ...
        if (recommendation.getStatus() != RecommendationStatus.DRAFT) {
            throw new IllegalArgumentException("Only DRAFT recommendations can be updated. "
                    + "Current status: " + recommendation.getStatus());
        }
    ...
    public void deleteRecommendation(Long id) {
        ...
        if (recommendation.getStatus() != RecommendationStatus.DRAFT) {
            throw new IllegalArgumentException("Only DRAFT recommendations can be deleted. "
                    + "Current status: " + recommendation.getStatus());
        }
