...
import com.wealthpro.analytics.feign.PborAccountClient;
...
public class OwnershipGuard {
    ...
    public void checkAccount(AuthContext ctx, Long accountId) {
        ...
        if (!ctx.isClient()) throw new ResponseStatusException(HttpStatus.FORBIDDEN, "Not authorised.");
        try {
            Long owner = pborAccountClient.getAccountOwner(accountId);
            if (!ctx.ownsClient(owner)) {
                throw new ResponseStatusException(HttpStatus.FORBIDDEN,
                        "You are not allowed to access analytics for account " + accountId + ".");
            }
